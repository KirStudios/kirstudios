try:
    ...
    def makevar(varname=None, varcontent=None, ow=0):
        ...
        if varname == None:
            ...
            return 'error - no variable name listed [1]'
        if varcontent == None:
            runtime_errors = runtime_errors + 1
            return 'error - no variable content listed [2]'
        if ow == None:
            runtime_errors = runtime_errors + 1
            return 'error - no protection type listed [3]'
        if ow == 0:
            for variable in app_variable_table:
                if varname == variable:
                    runtime_errors = runtime_errors + 1
                    return 'fault - a variable with this name already exists [4]'
            app_variable_table[varname] = varcontent
        elif ow == 1:
            app_variable_table[varname] = varcontent
        return 'success - the task has been completed [0]'
    def editvar(varname=None, varcontent=None, ur=0):
        ...
        if varname not in app_variable_table:
            runtime_errors = runtime_errors + 1
            return 'fault - the variable name listed does not exist [5]'
        if ur == 0:
            og_varcontent = app_variable_table[varname]
            if og_varcontent == varcontent:
                runtime_errors = runtime_errors + 1
                return 'fault - the edited content is the same as the current content [6]'
        app_variable_table[varname] = varcontent
        return 'success - the task has been completed [0]'

    def renvar(varname=None, newvarname=None, ur=0):
        '''Edits a variable value'''
        global runtime_errors
        if varname == None:
            runtime_errors = runtime_errors + 1
        ...
        if newvarname == None:
            runtime_errors = runtime_errors + 1
            return 'error - the new variable name was not listed [7]'
        if varname not in app_variable_table:
            runtime_errors = runtime_errors + 1
            return 'fault - the variable name listed does not exist [5]'
        varcontent = app_variable_table[varname]
        if ur == 0:
            if newvarname == varname:
                runtime_errors = runtime_errors + 1
                return 'fault - the edited variable name is the same as the current variable name [8]'
            if newvarname in app_variable_table:
                runtime_errors = runtime_errors + 1
                return 'fault - a variable with this name already exists [4]'

        del app_variable_table[varname]
        app_variable_table[newvarname] = varcontent
    ...
    def viewvar(varname=None):
        ...
        if varname not in app_variable_table:
            runtime_errors = runtime_errors + 1
            return 'fault - the variable does not exist so it cannot be viewed [5]'
        for variable in app_variable_table:
            if variable == varname:
    ...
    def delvar(varname=None):
        ...
        if varname not in app_variable_table:
            runtime_errors = runtime_errors + 1
            return 'fault - the variable does not exist so it cannot be deleted [5]'
        for variable in app_variable_table:
            if variable == varname:
